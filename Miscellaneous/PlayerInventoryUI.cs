...
public class PlayerInventoryUI : MonoBehaviour
{
    ...
    [SerializeField] protected ScrollRect _backpackScrollRect = null;
    ...
    [Header("Tab Groups")]
    [SerializeField] protected List<InventoryUI_TabGroup> _tabGroups = new List<InventoryUI_TabGroup>();
	...
	[SerializeField] private Text		_MagText   		=	null;
	...
	[SerializeField] SharedInt            	_AmmoMag = null;
	[SerializeField] SharedInt            	_AmmoTotal = null;
            ...
            if (_pDAReferences._logEntries)
            {
                ...
                for (int i = 0; i < audioCount; i++)
                {
                    ...
                    else
                    {
                        ...
                        PlayerInventoryUI_PDAEntry pdaEntry = _pDAReferences._logEntries.GetChild(i).GetComponent<PlayerInventoryUI_PDAEntry>();
                        if (pdaEntry)
                        {
                            // And set the data
                            pdaEntry.SetData(audioItem, i);
    ...
    public void RefreshPDAEntries()
    {
        ...
        for (int i = 0; i < _pDAReferences._logEntries.childCount; i++)
        {
            ...
            PlayerInventoryUI_PDAEntry entry = _pDAReferences._logEntries.GetChild(i).GetComponent<PlayerInventoryUI_PDAEntry>();
            InventoryItemAudio audioItem = _inventory.GetAudioRecording(i);
    ...
    protected virtual void Initialize()
    {
		...
		_MagText.text = _AmmoMag.value.ToString();
		_AmmoText.text = _AmmoTotal.value.ToString();
    ...
    public void OnEnterTabZero(int index)
    {
        ...
        if (index >= 0 && index < tabGroup.Items.Count)
        {
            ...
            {
                tabGroup.Items[index].TabText.color = ( tabGroup.ActiveItem != index) ? tabGroup.TextHover : tabGroup.TextColor;
            }
        }
    }
    ...
    public void OnExitTabZero(int index)
    {
        ...
        if (index >= 0 && index < tabGroup.Items.Count)
        {
            if (tabGroup.Items[index].TabText != null)
            {
                tabGroup.Items[index].TabText.color = ( tabGroup.ActiveItem == index) ? tabGroup.TextColor : tabGroup.TextInactive;
            }
        }
    }
